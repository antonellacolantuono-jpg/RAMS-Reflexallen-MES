...
### TODO-056 — Multi-level timer aggregation on WO BO Steps tab

**Discovered**: 2026-05-03 (during PROMPT_DESIGN_ALIGNMENT D3 batch 9 implementation)
**Status**: 🟢 PENDING — frontend ships per-step elapsed timers on the Steps tab; group / phase / WO-level roll-up deferred.
**File**: `apps/web/src/app/(registries)/work-orders/[id]/page.tsx` (Steps tab) + new aggregation helper in `apps/web/src/lib/`
**Symptom**: The mockup `ScreenWO` Steps tab shows a hierarchical step list with elapsed-time chips at every level (step row + group row collapsible header + phase row + WO header). D3 Batch 9 ships per-step timers correctly (computed from `StepExecution.startedAt` / `completedAt`), but the parent rows show static counts only (e.g. "Phase 2 — 9 steps") with no aggregated time. Aggregation requires walking `WorkflowSnapshot` tree + `StepExecution[]` and summing `(completedAt - startedAt)` recursively, then keeping running totals live for in-progress phases.
**Acceptance criterion**:
- New `apps/web/src/lib/wo-timer-aggregation.ts` exports `aggregateStepTimers(snapshot, executions): Map<NodeId, ElapsedMs>` that walks the snapshot tree and rolls up per-step elapsed time to group / phase / WO levels.
- WO BO Steps tab uses the aggregation map to populate group-row + phase-row + WO-header chips.
- For in-progress steps (status='running'), elapsed = `now - startedAt`. Updates live via TanStack Query refetch on tab focus or 5s interval (whichever the demo prep needs).
...
**Resolved**: 2026-05-03 (PROMPT_DESIGN_ALIGNMENT D4 closure)
**Commit**: _D4 closure commit_
**Root cause**: `apps/web/src/app/(registries)/page.tsx` exported `deriveEquipmentCounts` + `deriveBoxCounts` alongside its default page component. Next.js 14 App Router only allows specific named exports from page files; the auto-generated `.next/types/app/(registries)/page.ts` enforces this via an `OmitWithTag` index-signature check, blocking `pnpm --filter @mes/web build`.
**Fix**: Moved both helpers + their bucket configs (`EQUIPMENT_BUCKETS`, `BOX_BUCKETS`) to `apps/web/src/lib/dashboard-helpers.ts`. Updated page.tsx to import them; updated test to import from the helper module path. Build now passes (`Compiled successfully`, exit 0, 33/33 routes generated). Discovery during PROMPT_DESIGN_ALIGNMENT D3 batch 7.2 verification — surfaced when `.next/types` was regenerated by a build attempt.

### TODO-039 — Design token migration `bg-primary-*` / `bg-success-*` / `text-primary-*` unmapped in apps/web ✅

**Resolved**: 2026-05-03 (PROMPT_DESIGN_ALIGNMENT D2 Batch 2)
**Commit**: `4fa80e2`
**Root cause**: pre-existing pages used legacy `bg-primary-*` / `bg-success-*` / `text-primary-*` Tailwind classes that were never mapped in `apps/web/tailwind.config.ts`. PROMPT_3d D5 hotfix only fixed the 4 dialogs.
**Fix**: D2 Batch 2 extended the apps/web tailwind config (Recommended Option A from the original TODO) and aligned the registry list pages to mockup tokens. Verified rendering across all list pages.
