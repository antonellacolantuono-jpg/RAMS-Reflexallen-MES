...
function snapshotToWorkflowModel(
  snap: WorkOrderSnapshotProjection,
  ...
  return {
    ...
    code: 'snapshot',
    name: `Snapshot v${snap.workflowVersionNumber}`,
    plantId: '',
    version: snap.workflowVersionNumber,
    ...meta,
    currentVersion: {
      id: workflowVersionId,
      workflowId: workflowSnapshotId,
      version: snap.workflowVersionNumber,
      status: 'approved',
      ...meta,
...
function WorkflowSnapshotTab({ wo }: { wo: WorkOrderDetailModel }) {
  ...
  const adapted = snapshotToWorkflowModel(
    snap.snapshotData,
    snap.id,
    snap.workflowVersionId,
    snap.createdAt,
  )
  return (
    <div className="flex flex-col gap-2 h-full min-h-0">
      <div className="rounded-md border border-line bg-accent-soft text-accent-ink p-2.5 text-sm flex items-center gap-2 shrink-0">
        <span aria-hidden>🔒</span>
        Snapshot bloccato al rilascio · v{snap.snapshotData.workflowVersionNumber} ·{' '}
        {formatDateTimeIt(snap.createdAt)}
      </div>
      <div className="flex-1 min-h-[320px] rounded-md border border-line bg-paper overflow-hidden">
        <WorkflowHierarchyTable workflow={adapted} readOnly />
      </div>
    </div>
  )
}
