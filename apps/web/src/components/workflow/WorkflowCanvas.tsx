  ...
  phases.forEach((phase, phaseIdx) => {
    ...
    phase.groups.forEach((group, groupIdx) => {
      ...
      group.steps.forEach((step, stepIdx) => {
        // PROMPT_7 D1 — hydrate session-only fields (recoveryConfig, photoUrl,
        // configurator-driven actionType override) from the persisted JSON
        // blob on Step.data. Tolerant to malformed JSON (treat as absent).
        const parsedData = parseStepData(step.data ?? null)
        nodes.push({
          id: step.id,
          type: 'stepNode',
          position: { x: 0, y: 0 },
          data: {
            ...
            ...(parsedData?.recoveryConfig ? { recoveryConfig: parsedData.recoveryConfig } : {}),
            ...(parsedData?.photoUrl ? { photoUrl: parsedData.photoUrl } : {}),
          },
        })
