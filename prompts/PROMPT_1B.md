...
> **Predecessor**: PROMPT_1 (initial foundation — partial)
> **Goal**: Close the gaps in PROMPT_1 so the foundation is verifiably complete and ready for PROMPT_2.
> **Scope**: Surgical. Do NOT introduce new features.
> **Time budget**: 1.5–2 hours
> **Definition of Done**: see `prompts/DOD_TEMPLATE.md`. All checks must pass before this prompt is closed.
...
2. Locate or create the seed script per `MOCK_DATA_PNEUMATIC_AIR.md`. Wire it as `pnpm seed` in root `package.json`. Seed must be **idempotent** (use `prisma.<model>.upsert` keyed on stable business IDs, never on auto-increment ID).
...
- `docs: realign STATUS.md after PROMPT_1B (D6)`
